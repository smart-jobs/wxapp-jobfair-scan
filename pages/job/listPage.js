...
Page({
  ...
  getJobInList:function(message){
    ...
    var that = this;
    wx.request({
      url: app.globalData.contentpath + 'wechat/testlist', //仅为示例
      data: {
      ...
      success(res) {
        ...
        if (res.data.rescode == '1') {
          console.log(res.data.jobInList)
          that.setData({
            jobInList: res.data.jobInList
          })
        }
        if (that.data.pageNum >= res.data.pageCount) {
          that.setData({
            contentlist: res.data.jobInList,
            hasMoreData: false
          })
        } else {
          that.setData({
